...
def get_adj_windowed_obs(obs, query, indexer, adj = 10, set_k=1):
    obs_list = obs.split("\n")
...
if __name__=='__main__':
    ...
    with open(f"subquery_mind2web.csv", mode=mode, newline='') as file:
        ...
        for i, row in tqdm(enumerate(mind2web_data.iterrows()), total=mind2web_data.shape[0]):
            # if i > 100:  # read only the first 100 rows
            #     break
            ...
            results, windowed_obs, bounded_obs = get_windowed_obs(obs, query, indexer, adj=10, set_k_adj=3, set_k_bound=5)
    ...
    adj_accuracy = correct_count_adj / total_count
    bound_accuracy = correct_count_bound / total_count
    print(f"The final accuracy for adjacency window is: {adj_accuracy}") 
    print(f"The final accuracy for bounded window is: {bound_accuracy}")
